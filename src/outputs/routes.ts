...
function reviewNextRegistration(value: unknown): ReviewNextRegistration | undefined {
...
export function mountOutputsRoutes(
  ...
  app.post('/artifacts/register', async (req: Request, res: Response) => {
    try {
      ...
      if (payload.review_next) {
        await upsertReviewNextMetadata(adapter, row.artifact_id, payload.review_next, nowIso);
      }
