...
export function createDb(): Db {
  ...
  if (!connectionString) {
    throw new Error('DATABASE_URL is not set (required for DB persistence)');
  }
...
export async function migrateDb(db: Db): Promise<void> {
  ...
  await db.pool.query(`ALTER TABLE teams ADD COLUMN IF NOT EXISTS port_start integer NOT NULL DEFAULT 4101;`);
  await db.pool.query(`ALTER TABLE teams ADD COLUMN IF NOT EXISTS port_end integer NOT NULL DEFAULT 4125;`);
  ...
  try {
      const dup = await db.pool.query<{ port_start: number; port_end: number; c: string }>(
    ...
    if (shouldReassign) {
      const teams = await db.pool.query<{ id: string }>(
        `SELECT id
         FROM teams
      ...
      for (const row of teams.rows) {
        const maxPortRes = await db.pool.query<{ max_port: number | null }>(
        ...
        await db.pool.query(`UPDATE teams SET port_start = $2, port_end = $3 WHERE id = $1`, [
          row.id,
          desiredStart,
          desiredEnd
        ]);
  ...
  await db.pool.query(`ALTER TABLE agents ADD COLUMN IF NOT EXISTS runtime text DEFAULT 'claude-agent-sdk';`);
