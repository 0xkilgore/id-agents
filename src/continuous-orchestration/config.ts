...
export interface ContinuousOrchestrationConfig {
  ...
  /** Enable the bounded lifecycle repair loop. Default false for staged rollout. */
  lifecycle_reconciliation_enabled: boolean;
...
export function loadContinuousOrchestrationConfig(
  ...
  return {
    ...
    lifecycle_reconciliation_enabled: envBool(
      env.CONTINUOUS_ORCHESTRATION_LIFECYCLE_RECONCILIATION_ENABLED,
      d.lifecycle_reconciliation_enabled,
    ),
