...
describe('deploy-freshness-watchdog source — clean deploy checkout hygiene', () => {
  ...
  it('routes launchd manager restart to the deploy checkout', () => {
    ...
    expect(WATCHDOG_SRC).toContain('bootstrapWithRollback');
    expect(WATCHDOG_SRC).toContain('restoreManagerPlist(previousPlist)');
  });

  it('makes bootstrap rollback and rollback failure paths visible', () => {
    expect(WATCHDOG_SRC).toContain("writeArtifact('rollback'");
    expect(WATCHDOG_SRC).toContain('REDEPLOY ROLLED BACK');
    expect(WATCHDOG_SRC).toContain('forward bootstrap failed and rollback bootstrap failed');
    expect(WATCHDOG_SRC).toContain('process.exitCode = 1');
  });
...
describe('manager-promote-rebuild-restart source — clean deploy checkout hygiene', () => {
  ...
  it('reloads launchd from the deploy checkout plist configuration', () => {
    ...
    expect(WRAPPER_SRC).toContain('LAUNCHCTL_BIN=${LAUNCHCTL_BIN:-launchctl}');
    expect(WRAPPER_SRC).toContain('LAUNCHD_DOMAIN="gui/$(id -u)"');
    expect(WRAPPER_SRC).toContain('"$LAUNCHCTL_BIN" bootstrap "$LAUNCHD_DOMAIN" "$PLIST"');
  });
});
