...
The `admin-control` skill lets any Claude Code session programmatically manage an ID Agents team. It provides scripts for sending CLI commands via HTTP, chatting with the manager, and polling for multi-agent replies — all without opening the interactive CLI.

## When to Use

Use admin-control when you want a Claude Code agent (or any script) to:
...
The skill includes shell scripts and Node.js helpers in `skills/admin-control/`:
...
./skills/admin-control/remote-command.sh "/deploy idchain"
./skills/admin-control/remote-command.sh "/ask coder Build a REST API"
```
...
```bash
./skills/admin-control/talk-to-manager.sh "Can I spawn 3 agents?" http://localhost:4050
```
...
```bash
node skills/admin-control/start-listener.js 4050
```
...
node skills/admin-control/admin-session.js talk "Can I spawn a new agent?"

# Execute a remote command
node skills/admin-control/admin-session.js remote "/agents"

# Start listener only
node skills/admin-control/admin-session.js listen
```
...
```bash
./skills/admin-control/management-loop.sh coder "Review open PRs" 60
```
...
```bash
./skills/admin-control/talk-to-manager.sh "I want to refactor the auth module. OK?" http://localhost:4050
```
...
```bash
./skills/admin-control/remote-command.sh "/ask coder Refactor the auth module"
```
...
node skills/admin-control/admin-session.js talk "Team deployed. Should I start the sprint?"

# 4. Dispatch tasks to agents
./skills/admin-control/remote-command.sh "/ask contracts Write tests for IDRegistry"
./skills/admin-control/remote-command.sh "/ask web Build the dashboard page"

# 5. Check progress
./skills/admin-control/remote-command.sh "/news contracts"
./skills/admin-control/remote-command.sh "/news web"
```
